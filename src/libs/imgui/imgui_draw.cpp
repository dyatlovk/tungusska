...
{
    ImDrawCmd* current_cmd = CmdBuffer.Size != 0 ? &CmdBuffer.back() : nullptr;
...
void ImDrawList::UpdateClipRect()
{
    ...
    if (curr_cmd->ElemCount == 0 && (prev_cmd != 0) && memcmp
                                                           (&prev_cmd->ClipRect, &curr_clip_rect, sizeof(ImVec4)) == 0 && prev_cmd->TextureId == GetCurrentTextureId() && prev_cmd->UserCallback == nullptr)
        CmdBuffer.pop_back();
    else
...
void ImDrawList::UpdateTextureID()
{
    ...
    ImDrawCmd* curr_cmd = CmdBuffer.Size != 0 ? &CmdBuffer.back() : nullptr;
    ...
    // Try to merge with previous command if it matches, else use current command
    ImDrawCmd* prev_cmd = CmdBuffer.Size > 1 ? curr_cmd - 1 : nullptr;
    if ((prev_cmd != 0) && prev_cmd->TextureId == curr_texture_id && memcmp(&prev_cmd->ClipRect, &GetCurrentClipRect(), sizeof(ImVec4)) == 0 && prev_cmd->UserCallback == nullptr)
        CmdBuffer.pop_back();
    else
...
void ImDrawList::PushClipRect(ImVec2 cr_min, ImVec2 cr_max, bool intersect_with_current_clip_rect)
{
    ...
    if (intersect_with_current_clip_rect && (_ClipRectStack.Size != 0))
    {
...
void ImDrawList::PathRect(const ImVec2& a, const ImVec2& b, float rounding, int rounding_corners)
{
    ...
    {
        const float r0 = (rounding_corners & ImGuiCorner_TopLeft) != 0 ? r : 0.0f;
        const float r1 = (rounding_corners & ImGuiCorner_TopRight) != 0 ? r : 0.0f;
        const float r2 = (rounding_corners & ImGuiCorner_BottomRight) != 0 ? r : 0.0f;
        const float r3 = (rounding_corners & ImGuiCorner_BottomLeft) != 0 ? r : 0.0f;
...
void ImDrawList::AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    ...
    font->RenderText(this, font_size, pos, col, clip_rect,
